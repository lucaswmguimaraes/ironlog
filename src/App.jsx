...
export default function App(){
  ...
  useEffect(() => {
    ...
    loadFromGitHub(profile.id, pat).then((githubData) => {
      if (githubData && githubData.length > 0) {
        lastSavedJsonRef.current = JSON.stringify(githubData);
        setSessions(githubData);
      } else {
  ...
  // Sync to GitHub — fires on sessions change, but skips if loading or data unchanged
  useEffect(() => {
    if (!profile) return;
      ...
      {showPicker && (
        ...
        <div style={S.modal} onClick={() => setShowPicker(false)}>
          ...
          <div style={S.mBox} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ ...S.cScroll, padding: "8px 14px 12px" }}>{cats.map(c => <button key={c} style={{ ...S.chip, ...(cat === c ? S.chipA : {}) }} onClick={() => setCat(c)}>{c}</button>)}</div>
...
const S = {
  ...
  cScroll: { display: "flex", gap: 8, overflowX: "auto", padding: "8px 4px 12px 4px", scrollbarWidth: "none", msOverflowStyle: "none", minHeight: 52 },

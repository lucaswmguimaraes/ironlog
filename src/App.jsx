...
export default function App(){
  ...
  useEffect(() => {
    ...
    loadFromGitHub(profile.id, pat).then((data) => {
      ...
      if (data && data.length > 0) {
        ...
        // No GitHub data — mark current localStorage state as "saved" so save doesn't fire

        lastSavedJsonRef.current = JSON.stringify(sessionsRef.current);

      }

      // Delay clearing the loading flag so React can process setSessions()
      // and run the sync effect BEFORE we allow saves.
      // Without this, the sync effect sees old sessions state vs new lastSavedJsonRef
      // and triggers a spurious save on every app open.
  ...
  useEffect(() => {
    ...
    // Allow saving empty array only if we had data before (user deleted everything)

    if (sessions.length === 0 && lastSavedJsonRef.current === null) return;
...
const S = {
  ...
  cScroll: { display: "flex", gap: 8, overflowX: "auto", padding: "6px 0 10px 0", scrollbarWidth: "none", msOverflowStyle: "none" },
